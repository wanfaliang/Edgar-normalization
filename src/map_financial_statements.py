...
def map_bs_item(plabel, line_num, control_lines):
    ...
    if line_num <= total_current_assets:
        ...
        if 'cash' in p and 'restricted' not in p and 'cash and short-term' not in p and 'cash and short term' not in p:
        ...
        if ('trade' in p and ('receivable' in p or 'receivables' in p)) or \
           ...
           (('note' in p or 'notes' in p) and ('receivable' in p or 'receivables' in p)):
    ...
    # NON-CURRENT ASSETS
        ...
        if 'goodwill' in p and 'intangible' not in p and 'intangibles' not in p:
            return 'goodwill'
        # CSV line 15: [intangible OR intangibles] NOT [goodwill]
        if ('intangible' in p or 'intangibles' in p) and 'goodwill' not in p:
        ...
        if 'deferred' in p and ('tax' in p or 'taxes' in p) and 'asset' in p:
            return 'tax_assets'
        if p in ['total assets', 'assets total'] or ('total' in p and 'asset' in p and 'current' not in p):
            return 'total_assets'
        if 'other' in p and ('non current' in p or 'long term' in p) and 'asset' in p:
    ...
    # LIABILITIES AND EQUITY (after total_assets)
        ...
        if ('accumulated' in p or 'retained' in p) and 'earning' in p:
        ...
        if ('treasury stock' in p or 'treasury stocks' in p) and ('cost' in p or 'par' in p):
        ...
        if line_num <= total_current_liabilities:
            ...
            if 'employment' in p or 'compensation' in p or 'wages' in p or 'salaries' in p or 'payroll' in p:
            ...
            if (('borrowing' in p or 'borrowings' in p or 'debt' in p or 'note' in p or 'notes' in p or 'loan' in p or 'loans' in p) and
               ...
               ('current maturities' in p or 'current portion' in p):
                return 'short_term_debt'
            # CSV line 30: [unearned OR unexpired]
            if 'unearned' in p or 'unexpired' in p:
                return 'deferred_revenue'
            # CSV line 29: [(payables OR payable) AND income taxes]
            if ('payable' in p or 'payables' in p) and 'income tax' in p:
                return 'tax_payables'
            ...
            if ('current' in p or 'short term' in p) and ('operating' in p) and ('lease' in p or 'leases' in p):
        ...
        # Non-current liabilities
            ...
            if ('note' in p or 'notes' in p or 'borrowing' in p or 'debt' in p) and ('long term' in p or 'non current' in p):
            ...
            if ('deferred revenue' in p or 'unearned' in p) and ('long term' in p or 'non current' in p):
            ...
            if 'long term' in p and 'income tax' in p:
                return 'deferred_tax_liabilities_non_current'
            ...
            if p in ['total liabilities', 'liabilities total'] or ('total' in p and 'liabilit' in p and 'current' not in p and 'stockholder' not in p and 'equity' not in p):
                return 'total_liabilities'
            if 'other' in p and ('non current' in p or 'long term' in p) and 'liabilit' in p:
    ...
    for item in line_items:
        ...
        if 'revenue' not in control_lines:
            if ('total revenue' in p or 'net revenue' in p or 'net revenues' in p or 'revenues' in p or \
               ...
               'total net sales and revenue' in p or ('total' in p and 'revenues' in p) or 'revenue'== p) and ('marketing' not in p and 'admini' not in p and 'general' not in p):
        ...
        if 'income_tax_expense' not in control_lines:
            if ('taxes' in p or 'tax' in p) and ('provision' in p or 'benefit' in p or 'expense' in p or 'expenses' in p or 'on' in p or 'income' in p):
...
def map_cf_item(plabel, line_num, control_lines, tag=''):
...
def map_statement(stmt_type, line_items, control_lines):
    ...
    for item in line_items:
        ...
        if stmt_type == 'BS':
            ...
            # Auto-assign unmapped items to other_*
            if not target:
                if section == 'current_assets':
                    target = 'other_current_assets'
                elif section == 'non_current_assets':
            ...
            section = classify_cf_section(line_num, control_lines)
            tag = item.get('tag', '')
            target = map_cf_item(plabel, line_num, control_lines, tag)
...
def get_balance_sheet_structure():
    ...
    return [
        ...
        {'type': 'item', 'field': 'operating_lease_obligations_non_current', 'label': 'Operating lease liabilities', 'indent': 1},
        ...
        {'type': 'item', 'field': 'common_stock', 'label': 'Common stock and paid-in capital', 'indent': 1},
